...
import { Button, Paper, Stack, TextField, Typography, IconButton, Alert } from '@mui/material'
...
export default function KeyStore() {
  ...
  const generateKeystore = async () => {
    try {
      ...
      // Dynamic import ethers v6 only when needed
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      setKeystoreJson(event.target?.result as string)
      ...
      <Stack spacing={4}>
        <Paper elevation={3} sx={{ padding: '20px' }}>
          ...
          <Stack spacing={2}>
            ...
            <Button variant="contained" color="primary" onClick={generateKeystore}>Generate and Save Keystore</Button>
          </Stack>
        </Paper>
        ...
        <Paper elevation={3} sx={{ padding: '20px' }}>
          ...
          <Stack spacing={2}>
            ...
            {fileUploaded && <Alert severity="success">Keystore file uploaded successfully!</Alert>}
            <TextField label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
            <Button variant="contained" color="primary" onClick={decryptKeystore}>Decrypt Keystore</Button>
            {decryptSuccess && <Alert severity="success">Keystore decrypted successfully!</Alert>}
            {decryptedAddress && (
              <TextField
